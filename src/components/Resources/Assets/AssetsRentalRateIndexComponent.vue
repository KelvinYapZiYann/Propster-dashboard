<template>
  <div class="row">
    <div class="col-12">
      <card>
        <h4 slot="header" class="card-title text-left">{{$t('dashboard.assetsMarketRentalRate')}}</h4>
        <!-- <div class="row">
      ...
      try {
        await this.$store.dispatch('asset/get', pageId).then(() => {
            ...
            for (let i = 0; i < this.resource.models.length; i++) {
                this.resource.models[i]['current_rental_rate'] = '1250';
                this.resource.models[i]['current_market_rental_rate'] = '1350';
            }
